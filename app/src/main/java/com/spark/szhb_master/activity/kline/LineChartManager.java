...
import com.github.mikephil.chartingtwo.formatter.IAxisValueFormatter;
...
import com.spark.szhb_master.entity.DepthChart;
...
//        lineChart.setBackgroundColor(R.color.main_head_bg);
        //是否显示边界
...
//        rightYAxis.setEnabled(false);
     ...
     * @param color    曲线颜色
     */
    public void showLineChar(final List<DepthChart.BidBean.ItemsBeanX> dataList, String name, int color) {
        List<Entry> entries = new ArrayList<>();
        int j = dataList.size()-1;
        float amount = 0;
//        Collections.reverse(dataList);
        for (int i = 0;i < dataList.size(); i++) {
            DepthChart.BidBean.ItemsBeanX data = dataList.get(i);
            amount = amount + (int) data.getAmount();
            j = j -1;
            /**
             * 在此可查看 Entry构造方法，可发现 可传入数值 Entry(float x, float y)
             * 也可传入Drawable， Entry(float x, float y, Drawable icon) 可在XY轴交点 设置Drawable图像展示
             */
//            Entry entry = new Entry(j+1, amount);
            Entry entry = new Entry(i, amount);
            entries.add(entry);
...
//        xAxis.setLabelCount(6, false);
        ...
        xAxis.setValueFormatter(new IAxisValueFormatter() {
            @Override
            public String getFormattedValue(float value, AxisBase axis) {
                double tradeDate = dataList.get((int) value % dataList.size()).getPrice();
                return tradeDate + "";
            }
        });
    ...
    public void showLineChartwo(final List<DepthChart.AskBean.ItemsBean> dataList, String name, int color) {
        List<Entry> entries = new ArrayList<>();
        float amount = 0;
        for (int i = 0; i < dataList.size(); i++) {
//            for (int i = dataList.size()-1; i > 0; i-- ) {
            DepthChart.AskBean.ItemsBean data = dataList.get(i);
            amount = amount + (float) data.getAmount();
            /**
             * 在此可查看 Entry构造方法，可发现 可传入数值 Entry(float x, float y)
             * 也可传入Drawable， Entry(float x, float y, Drawable icon) 可在XY轴交点 设置Drawable图像展示
             */
